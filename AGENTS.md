...
- **v1.2.2:** CLI table collapses matching IPv4/IPv6 listeners by port, protocol, and PID; raw JSON and port inspection retain every address. `-a` is the short alias for `--all`.
- **v1.2.3:** **CLI collapses everywhere, with an `ADDRESS` column** — table, port inspection, and `--json` all emit one row per listener (grouped by port + protocol + PID) instead of one per bind address. The table gained an `ADDRESS` column after `PROTO` listing every address (`*` for a wildcard `0.0.0.0`/`::` bind); `--json` replaces `address` with an `addresses` array and drops the address from `id` (`tcp|5432|17364`). Table column widths now count characters, not bytes, so the `—` placeholder no longer misaligns rows. Docs: `-a` documented as the `--all` alias, README gained a CLI section.
...
**Shipped:** `main` = **v1.2.3** — the **Current features** list above is what's live.

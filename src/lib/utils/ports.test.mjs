...
import { brandIconUrl, brandSlug, filterPorts, groupPorts, iconSources, isDatabase } from "./ports.js";
...
test("search includes system and UDP ports without changing the default scope", () => {
  const dev = { ...port("dev", 1, 3000), address: "127.0.0.1", protocol: "tcp", isSystemPort: false };
  const tailscale = { ...port("tailscale", 2, 41641, "tailscaled.exe"), address: "0.0.0.0", protocol: "udp", isSystemPort: true };

  assert.deepEqual(filterPorts([dev, tailscale], false, false, ""), [dev]);
  assert.deepEqual(filterPorts([dev, tailscale], false, false, "tailscale"), [tailscale]);
  assert.deepEqual(filterPorts([dev, tailscale], false, false, "  "), [dev]);
  assert.deepEqual(filterPorts([dev, tailscale], false, false, "", [41641]), [dev, tailscale]);
  assert.deepEqual(groupPorts([dev, tailscale], [41641]), [[tailscale], [dev]]);
});

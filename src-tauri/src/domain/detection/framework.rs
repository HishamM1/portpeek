    ...
    detect_by_unique_process_name(pname)
        ...
        .or_else(|| {
            ...
            root.map(config_candidates).and_then(|candidates| {
                candidates
                    .into_iter()
                    .find(|d| runtime_matches(pname, cmd, &d.name))
            })
        })
        .or_else(|| {
            command
...
fn runtime_matches(process_name: &str, command: &str, framework: &str) -> bool {
    ...
    if pn.contains(&framework.to_ascii_lowercase()) || cmd.contains(&framework.to_ascii_lowercase())
    {
        return true;
    }

    match framework {
        "Next.js" | "Nuxt" | "SvelteKit" | "Vite" | "React" | "Vue" | "Svelte" | "Node.js"
        | "Express" | "NestJS" | "Astro" | "Fastify" | "Hapi" | "Koa" | "AdonisJS" | "Remix" => {
            ["node", "bun", "deno", "npm", "pnpm", "yarn"]
                .iter()
                .any(|r| pn.contains(r) || cmd.contains(r))
        }
        "Django" | "FastAPI" | "Flask" | "Uvicorn" | "Gunicorn" | "Streamlit" | "Jupyter"
        | "Hypercorn" | "Gradio" | "Python" => pn.contains("python") || cmd.contains("python"),
        ...
        "Go" | "Gin" | "Fiber" | "Echo" => pn.contains("go.exe") || cmd.contains("go run"),
        "Java" | "Spring Boot" | "Apache Tomcat" | "Quarkus" | "Micronaut" | "Jetty"
        | "WildFly" | "Ktor" | "Play Framework" | "Grails" => {
...
fn detect_by_unique_process_name(process_name: &str) -> Option<FrameworkDetection> {
    ...
    let result = match name {
        ...
        "iisexpress" | "iisexpresstray" => Some(("IIS Express", FrameworkConfidence::High)),
        "w3wp" => Some(("ASP.NET", FrameworkConfidence::High)),
        "sqlservr" => Some(("SQL Server", FrameworkConfidence::High)),
        "rabbitmq-server" | "rabbitmq" => Some(("RabbitMQ", FrameworkConfidence::High)),
...
fn config_candidates(root: &Path) -> Vec<FrameworkDetection> {
    ...
    for (files, name) in flat_candidates {
        if files.iter().any(|file| root.join(file).exists()) {
            push!(name, FrameworkConfidence::High);
        }
    }
    ...
    if let Some(pyproject) = read_small_file(&root.join("pyproject.toml")) {
        ...
        if text.contains("flask") {
            push!("Flask", FrameworkConfidence::High);
        }
        if text.contains("streamlit") {
            push!("Streamlit", FrameworkConfidence::High);
        }
        if text.contains("gradio") {
            push!("Gradio", FrameworkConfidence::High);
        }
        if text.contains("gunicorn") {
            push!("Gunicorn", FrameworkConfidence::Medium);
        }
    }
        ...
        .flatten()
    {
        ...
        if text.contains("sinatra") {
            push!("Sinatra", FrameworkConfidence::High);
        }
        if text.contains("puma") {
            push!("Puma", FrameworkConfidence::Medium);
        }
        if text.contains("rails") {
            push!("Rails", FrameworkConfidence::High);
        }
    }
        ...
        .flatten()
    {
        if let Ok(decoded) = serde_json::from_slice::<Value>(&composer) {
            ...
            if has("laravel/framework") || has("laravel") {
                push!("Laravel", FrameworkConfidence::High);
            }
            if has("symfony/framework-bundle") || has("symfony/symfony") {
                push!("Symfony", FrameworkConfidence::High);
            }
            if has("codeigniter/framework") {
                push!("CodeIgniter", FrameworkConfidence::High);
            }
            if has("wp-cli/wp-cli") {
                push!("WordPress", FrameworkConfidence::Medium);
            }
        }
    }
        ...
        .flatten()
    {
        ...
        if text.contains("fastapi") {
            push!("FastAPI", FrameworkConfidence::High);
        }
        if text.contains("django") {
            push!("Django", FrameworkConfidence::High);
        }
    }
        ...
        .flatten()
    {
        ...
        if text.contains("axum") {
            push!("Axum", FrameworkConfidence::High);
        }
        if text.contains("actix-web") || text.contains("actix_web") {
            push!("Actix Web", FrameworkConfidence::High);
        }
        if text.contains("rocket") {
            push!("Rocket", FrameworkConfidence::High);
        }
        if text.contains("warp") {
            push!("Warp", FrameworkConfidence::Medium);
        }
    }
        ...
        .flatten()
    {
        ...
        if text.contains("gin-gonic/gin") || text.contains("gin ") {
            push!("Gin", FrameworkConfidence::High);
        }
        if text.contains("gofiber/fiber") || text.contains("fiber ") {
            push!("Fiber", FrameworkConfidence::High);
        }
        if text.contains("labstack/echo") || text.contains("echo ") {
            push!("Echo", FrameworkConfidence::Medium);
        }
    }
        ...
        .flatten()
    {
        ...
        if text.contains("phoenix") {
            push!("Phoenix", FrameworkConfidence::High);
        }
    }
        ...
        })
    {
        ...
        if text.contains("flutter") {
            push!("Flutter", FrameworkConfidence::High);
        }
        if text.contains("shelf") {
            push!("Dart", FrameworkConfidence::Medium);
        }
    }
        ...
        .flatten()
    {
        ...
        if text.contains("spring-boot") {
            push!("Spring Boot", FrameworkConfidence::High);
        }
        if text.contains("quarkus") {
            push!("Quarkus", FrameworkConfidence::High);
        }
        if text.contains("micronaut") {
            push!("Micronaut", FrameworkConfidence::High);
        }
        if text.contains("tomcat") {
            push!("Apache Tomcat", FrameworkConfidence::High);
        }
        if text.contains("jetty") {
            push!("Jetty", FrameworkConfidence::High);
        }
        if text.contains("wildfly") || text.contains("jboss") {
            push!("WildFly", FrameworkConfidence::High);
        }
        if text.contains("play") {
            push!("Play Framework", FrameworkConfidence::Medium);
        }
        if text.contains("grails") {
            push!("Grails", FrameworkConfidence::High);
        }
    }
        ...
        })
    {
        ...
        if text.contains("ktor") {
            push!("Ktor", FrameworkConfidence::High);
        }
    }
        ...
        .flatten()
    {
        ...
        if text.contains("spring") {
            push!("Spring Boot", FrameworkConfidence::Medium);
        }
    }
    ...
    if let Some(csproj) = glob_first(root, "*.csproj") {
        let aspnet = read_small_file(&csproj)
            .map(|bytes| String::from_utf8_lossy(&bytes).to_ascii_lowercase())
            .is_some_and(|text| text.contains("microsoft.aspnetcore") || text.contains("kestrel"));
        if aspnet {
            push!("ASP.NET", FrameworkConfidence::High);
        }
        push!(".NET", FrameworkConfidence::Medium);
    }
    ...
    if let Some(name) = generic {
        push!(name, FrameworkConfidence::Medium);
    }
...
fn detect_command(command: &str) -> Option<FrameworkDetection> {
    ...
    // Match Next.js invocations, not any command that merely contains "next"
    // (e.g. nextcloud, nextdoor, C:\nextcloud\server.js). A listening Next.js
    // process always runs a subcommand, so key off those. ponytail: substring
    // matching is the ceiling here; a token/argv-aware matcher would remove the
    // whole class of path/name collisions.
    if cmd.contains("next dev")
        || cmd.contains("next start")
        || cmd.contains("next build")
        || cmd.contains("next.js")
    {
        return Some(found(
            "Next.js",
            FrameworkConfidence::Medium,
...
fn detect_generic_runtime(
    ...
    if pn.contains("java") || cmd.contains("java") {
        return Some(found(
            "Java",
            FrameworkConfidence::Low,
            FrameworkDetectionSource::Unknown,
        ));
    }
...
mod tests {
    ...
    fn detects_frameworks_from_process_name() {
        ...
        assert_eq!(detect_by_unique_process_name("dotnet.exe"), None);
        assert_eq!(detect_by_unique_process_name("node.exe"), None);
    }
    ...
    fn full_detection_pipeline_java_process() {
        let detection = detect(
            Some("java.exe"),
            Some("java -jar app.jar --spring-boot"),
            None,
        );
    ...
    fn full_detection_pipeline_node_process() {
        let detection = detect(
            Some("node.exe"),
            Some("node node_modules/.bin/next dev"),
            None,
        );
    ...
    fn process_name_matches_infrastructure_tools() {
        assert_eq!(
            detect_by_unique_process_name("redis-server.exe")
                .unwrap()
                .name,
            "Redis"
        );
        assert_eq!(
            detect_by_unique_process_name("postgres.exe").unwrap().name,
            "PostgreSQL"
        );
        assert_eq!(
            detect_by_unique_process_name("mysqld.exe").unwrap().name,
            "MySQL"
        );
        assert_eq!(
            detect_by_unique_process_name("prometheus.exe")
                .unwrap()
                .name,
            "Prometheus"
        );
        assert_eq!(
            detect_by_unique_process_name("grafana.exe").unwrap().name,
            "Grafana"
        );
        assert_eq!(
            detect_by_unique_process_name("vault.exe").unwrap().name,
            "HashiCorp Vault"
        );
        assert_eq!(
            detect_by_unique_process_name("consul.exe").unwrap().name,
            "HashiCorp Consul"
        );
        assert_eq!(
            detect_by_unique_process_name("traefik.exe").unwrap().name,
            "Traefik"
        );
        assert_eq!(
            detect_by_unique_process_name("envoy.exe").unwrap().name,
            "Envoy"
        );
    }
    ...
    fn config_detection_prefers_specific_frameworks_over_generic_markers() {
        let root =
            std::env::temp_dir().join(format!("portpeek-framework-test-{}", std::process::id()));
    ...
    #[test]
    fn config_detection_falls_back_to_generic_runtime_without_specific_markers() {
        let root =
            std::env::temp_dir().join(format!("portpeek-framework-generic-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(&root).unwrap();
        ...
        .unwrap();
        let java = detect_config(&root).unwrap();
    ...
    fn command_does_not_false_positive_on_next_substring() {
        ...
        for cmd in [
            ...
            "node C:\\nextcloud\\server.js",
            "node /opt/nextdoor/app.js",
        ] {
            assert_eq!(
                detect_command(cmd).unwrap().name,
                "Node.js",
                "mislabeled: {cmd}"
            );
        }
